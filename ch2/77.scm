...
;; If we let z be defined as
(define z (make-from-real-imag 3 4))
;; then the application (magnitude z) first dispatches on the
;; type 'complex and and then dispatches on the type 'rectangular,
;; so apply-generic is invoked twice. In both cases the dispatched
;; procedure is the same, only the type on which it operates is
;; different.
